...
const DentalChart = ({ patientId, initialData, onUpdate }) => {
  ...
  const handleToothClick = (number, e) => {
    ...
    if (!viewMode === 'structural' && toothData.full === 'missing') {
      ...
      console.warn('This tooth is marked as missing. Switch to structural view to modify.');
      return;
    }
  ...
  const renderToothSurfaces = (number, toothData = {}) => {
    const isAnterior = [13, 12, 11, 21, 22, 23, 43, 42, 41, 31, 32, 33].includes(number);
    ...
    return (
      <div className={`tooth-surfaces ${isMissing ? 'missing' : ''}`}>
        {!isLowerJaw ? (
          <>
            <div 
              className={`surface buccal ${toothData.buccal ? `procedure-${toothData.buccal}` : ''} ${isMissing ? 'disabled' : ''}`}
              onClick={(e) => handleSurfaceClick('buccal', e)}
            />
            <div 
              className={`surface mesial ${toothData.mesial ? `procedure-${toothData.mesial}` : ''} ${isMissing ? 'disabled' : ''}`}
              onClick={(e) => handleSurfaceClick('mesial', e)}
            />
            <div 
              className={`surface occlusal ${toothData.occlusal ? `procedure-${toothData.occlusal}` : ''} ${isMissing ? 'disabled' : ''}`}
              onClick={(e) => handleSurfaceClick('occlusal', e)}
            />
            <div 
              className={`surface distal ${toothData.distal ? `procedure-${toothData.distal}` : ''} ${isMissing ? 'disabled' : ''}`}
              onClick={(e) => handleSurfaceClick('distal', e)}
            />
            <div 
              className={`surface lingual ${toothData.lingual ? `procedure-${toothData.lingual}` : ''} ${isMissing ? 'disabled' : ''}`}
  ...
  const renderToothStructural = (number, toothData = {}) => {
    return (
      <div 
        className={`tooth-structural ${toothData.full === 'missing' ? 'missing' : ''}`}
        onClick={(e) => handleToothClick(number, e)}
      >
        <div 
          className={`crown ${toothData.crown ? `procedure-${toothData.crown}` : ''}`}
          onClick={(e) => {
            e.stopPropagation();
            setSelectedTooth({ number });
        ...
        <div className="root-container">
          <div 
            className={`root ${toothData.root ? `procedure-${toothData.root}` : ''}`}
            onClick={(e) => {
              e.stopPropagation();
              setSelectedTooth({ number });
          ...
          >
            {toothData.root === 'rootcanal' && <div className="root-canal-indicator" />}
            {toothData.root === 'implant' && <div className="implant-indicator" />}
          </div>
        </div>
  ...
  const renderJaw = (teeth, isUpper) => (
    <div className={`${isUpper ? 'upper' : 'lower'}-jaw`}>
      {teeth.map(number => {
        const toothData = chartData[number] || {};
        const hasComments = toothData.comments && Object.keys(toothData.comments).length > 0;
        
        return (
          <div 
            key={number}
            className={`tooth ${toothData.status ? `procedure-${toothData.status}` : ''} 
                       ${hasComments ? 'has-comments' : ''}`}
          >
            <div className="tooth-number">{number}</div>
            {renderTooth(number, toothData)}
